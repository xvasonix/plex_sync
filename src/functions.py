...
# ---------------------------------------------------------
# Prioritize Env Var -> Default to /config (Docker) -> Fallback to . (Local)
...
# Load Environment Variables from CONFIG_DIR
dotenv_path = os.path.join(CONFIG_DIR, ".env")
load_dotenv(dotenv_path, override=True)

config_dir = CONFIG_DIR # Alias for compatibility if needed locally in file
mark_env = os.getenv("MARK_FILE", os.getenv("MARKFILE"))

if mark_env and os.path.isabs(mark_env):
    mark_file = mark_env
elif mark_env:
    mark_file = os.path.join(CONFIG_DIR, mark_env)
else:
    mark_file = os.path.join(CONFIG_DIR, "mark.log")
